var TestHTMLRenderer = {
    ...
    _createRenderedJSON : function(id, object, label, order, rendering, open, hidden, hasSummary, cannotReRoot){
        return {
            ...
            "orderByNumber" : parseInt(label.replace( /^\D+/g, ''))
        };
    },
    ...
    renderColumn : function(columnObject, columnNumber) {
        var columnsArray = new Array(),
            ...
            open,
            hidden;
        columnsObjectsSplitted = columnObject.split(",");
        for (var i = 0; i < columnsObjectsSplitted.length; i++) {
            columnsArray.push(
                this._createRenderedJSON(
                    "id" + columnsObjectsSplitted[i],
                    columnsObjectsSplitted[i],
                    columnsObjectsSplitted[i],
                    (order || 0),
                    this.renderer.addAttribute(
    ...
    renderRow : function(rowObject, columnNumber) {
        var rowArray = new Array(),
            order,
            open,
            hidden;
        rowsObjectsSplitted = rowObject.split(",");
        for (var i = 0; i < rowsObjectsSplitted.length; i++){
            rowArray.push(
                this._createRenderedJSON(
                    "id" + rowsObjectsSplitted[i],
                    rowsObjectsSplitted[i],
                    rowsObjectsSplitted[i],
                    (order || 0),
                    this.renderer.addAttribute(
                        this.renderer.createElementWithText("th",
...
TestHTMLRenderer = Class.extend(enioka.ij.IIJRenderer, TestHTMLRenderer);
